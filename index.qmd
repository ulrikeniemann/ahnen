...
```{r}
# interessante Spalten der Ahnendatei
...
Davon sind `r ZahlDirektAhnen` Direkt-Ahnen.
...
data <- data |> 
  # Tod
  mutate(TODESDATUM = str_replace(TODESDATUM, "ABT ", "1 JAN ")) |> 
  mutate(TODESDATUM = str_replace(TODESDATUM, "FROM ", "1 JAN ")) |> 
  mutate(TODESDATUM = str_replace(TODESDATUM, "TO ", "1 JAN ")) |> 
  mutate(TODESDATUM = str_replace(TODESDATUM, "BEF ", "1 JAN ")) |> 
  mutate(TODESDATUM = str_replace(TODESDATUM, "AFT ", "1 JAN ")) |> 
  ...
  # Geburt
  mutate(GEBURTSDATUM = str_replace(GEBURTSDATUM, "ABT ", "1 JAN ")) |> 
  mutate(GEBURTSDATUM = str_replace(GEBURTSDATUM, "FROM ", "1 JAN ")) |> 
  mutate(GEBURTSDATUM = str_replace(GEBURTSDATUM, "TO ", "1 JAN ")) |> 
  mutate(GEBURTSDATUM = str_replace(GEBURTSDATUM, "BEF ", "1 JAN ")) |> 
  mutate(GEBURTSDATUM = str_replace(GEBURTSDATUM, "AFT ", "1 JAN ")) |> 
...
## Generation - nur Direktvorfahren

Sollen 1a und 1b zusammen gezählt werden?

```{r}
computeTable(data = data[!is.na(data$Generation), ], Generation) |> getTable()
#
dat <- computeTable(data = data[!is.na(data$Generation), ], Generation) |> filter(row_number() <= n()-1)
...
```{r}
birth <- makeIcon("https://github.com/ulrikeniemann/ahnen/data/birth.png", 18, 18)
death <- makeIcon("https://github.com/ulrikeniemann/ahnen/data/death.png", 18, 18)
# Karte V1: mit icons
leaflet(data) |>
  addTiles() |>
